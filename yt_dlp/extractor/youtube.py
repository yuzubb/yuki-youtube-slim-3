...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def _call_api(self, ep, query, video_id, fatal=True, headers=None,
        ...
        return self._download_json(
            r'https://yuki-proxy-84513.deno.dev/?target='+urllib.parse.quote(f'https://{self._select_api_hostname(api_hostname, default_client)}/youtubei/v1/{ep}'),
    ...
    def _extract_video(self, renderer):
        ...
        related_videos = []
        related_renderer = traverse_obj(renderer, ('relatedVideoRenderer', 'items'), default=[])
        for rel_video in related_renderer:
            related_video_id = rel_video.get('videoId')
            if related_video_id:
                related_videos.append({
                    'id': related_video_id,
                    'title': self._get_text(rel_video, 'title'),
                    'url': f'https://www.youtube.com/watch?v={related_video_id}',
                    'duration': int_or_none(rel_video.get('lengthSeconds'))
                })
